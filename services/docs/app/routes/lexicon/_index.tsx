...
export default function LexiconIndex() {
  ...
  const methods = entries.filter((e) => e.type !== "record");
  ...
  return (
    <div>
      ...
      <p className="text-text-muted dark:text-text-muted-dark mb-8">
        Complete API reference for the atiproto tipping and subscription service.
      </p>
          ...
          {records.map((entry) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span className="font-mono text-sm font-medium">{entry.nsid}</span>
                <TypeBadge type={entry.type} />
              </div>
              <p className="text-sm text-text-muted dark:text-text-muted-dark">{entry.description}</p>
            </Link>
          ))}
        </div>
      </section>
