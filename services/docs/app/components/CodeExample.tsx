...
function generateSampleValue(name: string, prop: Record<string, unknown>): string {
  const type = prop.type as string;
  const format = prop.format as string | undefined;
  const ref = prop.ref as string | undefined;

  if (format === "did") return '"did:plc:example123"';
  if (format === "at-uri") return '"at://did:plc:example123/com.example.post/abc"';
  ...
  if (name === "amount" || name === "minimumTip" || name === "total") return "500";
...
function buildObjectLiteral(
  ...
  const keys = onlyRequired && required.length > 0
...
export function CodeExample({
  ...
  const agentPath = nsid.replace(/^com\./, "").split(".").join(".");
  ...
  if (type === "record") {
    ...
    if (recordSchema) {
      lines.push(`const record = ${buildObjectLiteral(recordSchema, "", false)};`);
    }
    return (
