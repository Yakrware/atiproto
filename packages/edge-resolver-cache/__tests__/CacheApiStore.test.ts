...
import { CacheApiStore } from "../src/CacheApiStore.js";
...
describe("CacheApiStore", () => {
  ...
  it("returns undefined for missing keys", async () => {
    const store = new CacheApiStore({ ttlSeconds: 3600 });
    const result = await store.get("nonexistent");
    expect(result).toBeUndefined();
  });

  it("stores and retrieves a value", async () => {
    const store = new CacheApiStore({ ttlSeconds: 3600, prefix: "test:" });
  ...
  it("uses correct Cache-Control header for TTL", async () => {
    const store = new CacheApiStore({ ttlSeconds: 7200, prefix: "did:" });
  ...
  it("constructs correct URL with prefix and encoding", async () => {
    const store = new CacheApiStore({ ttlSeconds: 3600, prefix: "did:" });
  ...
  it("deletes a key", async () => {
    const store = new CacheApiStore({ ttlSeconds: 3600, prefix: "x:" });
  ...
  it("opens a named cache", async () => {
    const _store = new CacheApiStore({ cacheName: "my-cache", ttlSeconds: 60 });
